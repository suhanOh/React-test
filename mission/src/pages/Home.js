...
function MissionTaBle(props){
    ...
    for(let i = 0; i< props.missionTable.length; i++){
        ...
        if( table.page === props.page){
            if(table.checked){
                tableList.push(<tr className={table.gender}><td className="td1"><input type="checkbox" checked value={table.num} onClick={event=>{
                ...
                tableList.push(<tr className={table.gender}><td className="td1"><input type="checkbox"  value={table.num} onClick={event=>{
                    props.onCheck(table.checked ,event.target.value);
                }}/></td>
            <td className="td2" onClick={()=>{
...
function Add(props){
    ...
    for(let i =0; i< props.add; i++){
        ...
        <td><span><input type='radio' name={"gender"+i} id={"man"+i} value="남" onClick={event =>{
        ...
        }}/><label for={"man"+i} className="radiobtn">남</label>
            <input type='radio' name={"gender"+i} id={"woman"+i} value="여" onClick={event =>{
                let newRadio = [...props.radiobtn];
                newRadio[i] = event.target.value;
                props.addCityPick(props.inputBox[i],props.inputBox2[i],newRadio[i],props.addWorld[i],props.addCity[i], i);
        }}/><label for={"woman"+i} className="radiobtn">여</label></span></td>
...
function Popup(props){
    const world = ['국가','대한민국','미국','일본','중국'];
    const [newCitys, setNewCitys] = useState([]);
    const [popGender,setPopGender] = useState('');
    const [popworld, setpopworld] = useState('국가'); 
    const [popcity, setpopcity] = useState('도시'); 
    if(props.popupShow){
        ...
        <div class="white-bg">   
            ...
            <div class="popup-update">
                ...
                <input className="title" type="text" value="아이디" readonly/>
                <input className="poptb" type="text" value={props.table.id} readonly/><br/>
                <input className="title" type="text" value="이름" readonly/>
                <input className="poptb" type="text" value={props.name} onChange={event =>{
                    props.changeName(event.target.value);
                }}/><br/>
                <input className="title" type="text" value="성별" readonly/>
                <span> 
                    <input type="radio" className="popradio" name="pop-gender" id="pop-man" value="남" onClick={event =>{
                        setPopGender(event.target.value);
                    }}/><label className="label" for="pop-man">남</label>
                    <input type="radio" className="popradio" name="pop-gender" id="pop-woman" value="여" onClick={event =>{
                        setPopGender(event.target.value);
                    }}/><label className="label" for="pop-woman">여</label>
                </span><br/>
                <input className="title" type="text" value="국가" readonly/>
                ...
                <input className="title" type="text" value="도시" readonly/>
                ...
                <button type="button" className="btn" onClick={()=>{                    
                    if(popGender === '' || popworld === '국가' || popcity === '도시'){
...
const Home = () => {
    ...
    const world = ['국가','대한민국','미국','일본','중국'];
    const [pickworld,setPickworld] = useState('국가');
    const [pickcity,setPickcity] = useState('도시');
    ...
    const [missionTables, setMissionTables] = useState([
        {num: 0, id:'hong', name:'길동', gender: '남', world: '대한민국', city: '서울', checked: false, date: '2023-01-10', page: 0},
        {num: 1, id:'gang', name:'철수', gender: '남', world: '미국', city: '뉴욕', checked: false, date: '2023-01-12', page: 0},
        {num: 2, id:'kim', name:'영희', gender: '여', world: '대한민국', city: '수원', checked: false, date: '2023-01-11', page: 0},
        {num: 3, id:'park', name:'길동', gender: '남', world: '중국', city: '베이징', checked: false, date: '2023-01-10', page: 0},
        {num: 4, id:'lee', name:'길숙', gender: '여', world: '일본', city: '도쿄', checked: false, date: '2023-01-09', page: 0},
        {num: 5, id:'choi', name:'철수', gender: '남', world: '대한민국', city: '부산', checked: false, date: '2023-01-13', page: 1},
        {num: 6, id:'ga', name:'길동', gender: '남', world: '일본', city: '오사카', checked: false, date: '2023-01-11', page: 1},
        {num: 7, id:'na', name:'영희', gender: '여', world: '중국', city: '상하이', checked: false, date: '2023-01-16', page: 1},
        {num: 8, id:'da', name:'철수', gender: '남', world: '대한민국', city: '대전', checked: false, date: '2023-01-15', page: 1},
        {num: 9, id:'ra', name:'길숙', gender: '여', world: '미국', city: '라스베이거스', checked: false, date: '2023-01-14', page: 1},
        {num: 10, id:'ma', name:'길동', gender: '남', world: '대한민국', city: '서울', checked: false, page: 2},
    ]);
    ...
    const [addWorld,setAddWorld] = useState(['국가','국가','국가']);
    const [addCity,setAddCity] = useState(['도시','도시','도시']);
    ...
    return<div>
        ...
        <from>
            <div className="mission">
                <input type='text' className="inputbox" placeholder="아이디" onChange={event=>{
                    setId(event.target.value);
                }}/>
                <input type='text' className="inputbox" placeholder="이름" onChange={event =>{
                    setName(event.target.value);
                }}/>
                <span><input type='radio' name="gender" id="man" value='남' onClick={event =>{
                    setGender(event.target.value);
                }}/><label for="man" className="radiobtn">남</label>
                <input type='radio' name="gender" id="woman" value='여' onClick={event =>{
                    setGender(event.target.value);
                }}/><label for="woman" className="radiobtn">여</label></span>
                <span><Link to="/"><input type='radio' name="lang" id="ko"/></Link><label for="ko" className="radiobtn">한국어</label>
